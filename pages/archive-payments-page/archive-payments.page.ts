...
// Define os formatos de dados aceitos pelos modais da página de pagamentos arquivados
...
export class ArchivePaymentsPage implements OnInit, OnDestroy {
  // ==========================================
  ...
  private fetchArchivePayments(showLoading = false): void {
    ...
    this.paymentService
      ...
      .subscribe({
        next: (response: any) => {
          const rawData: any[] = response?.payments ?? [];
          const budgetAllocation = response?.budget_allocation ?? null;

          const archivedPayments = rawData.map((item) =>
            this.mapPaymentRow(item, budgetAllocation)
          );

          this.archivedDataSource.data = archivedPayments;
        },
        error: () => {
  ...
  private mapPaymentRow(item: any, budgetAllocation: any) {
    return {
      ...item,
      name: item.patient_request?.report?.patient_care?.patient?.name || 'Não informado',
